...
function ScheduledPage({ layouts, onError, onMessage }: { layouts: Layout[]; onError: (value: string | null) => void; onMessage: (value: string | null) => void }) {
  const [tasks, setTasks] = useState<ScheduledTask[]>([]); const [draft, setDraft] = useState(blankSchedule); const [editing, setEditing] = useState<ScheduledTask | null>(null); const [loading, setLoading] = useState(true);
  async function refresh() { setLoading(true); try { setTasks(await api.listScheduled()); } catch (e) { onError(e instanceof Error ? e.message : "Could not load schedules"); } finally { setLoading(false); } }
  useEffect(() => { refresh(); }, []);
...
function nextRunLabel(task: ScheduledTask) { return task.frequency === "one_time" && new Date(task.run_at).getTime() <= Date.now() ? "Never" : formatTime(task.run_at); }
...
function RunSummary({ run }: { run: ComparisonRun }) { const metrics = [["Old file rows", run.old_row_count], ["New file rows", run.new_row_count], ["Deleted", run.removed], ["Changed", run.modified], ["Added", run.added]]; return <header className="mb-6"><h1 className="font-display text-3xl font-semibold">{run.run_name}</h1><p className="mt-1 text-[var(--muted)]">Run index {run.run_index} · Created {formatTime(run.created_at)} · Processing time {formatDuration(run.processing_duration_ms)}</p><div className="mt-4 grid gap-3 md:grid-cols-2"><SourceCard label="Old" file={run.old_origin_file_name} layout={run.old_layout_name} date={run.old_date_of_download}/><SourceCard label="New" file={run.new_origin_file_name} layout={run.new_layout_name} date={run.new_date_of_download}/></div><section className="mt-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4"><h2 className="font-display text-lg font-semibold">Comparison summary</h2><p className="mt-1 text-sm text-[var(--muted)]">Rows loaded from each file and the resulting changes.</p><dl className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-5">{metrics.map(([label, value]) => <div className="rounded-lg bg-[var(--surface-2)] p-3" key={label as string}><dt className="text-xs text-[var(--muted)]">{label}</dt><dd className="mt-1 text-xl font-semibold">{value ?? "—"}</dd></div>)}</dl></section></header>; }
