...
`old_rows`, `new_rows`, and `delta_rows` are stable PostgreSQL tables. Every parsed record stores its variable layout fields in a JSONB `data` payload, while its row ID, comparison ID, and composite primary key are normal typed columns with indexes. Download timestamp and source filename are load-level metadata held once on `comparison_runs`. This supports a different layout for every load without generating or migrating arbitrary database columns.
